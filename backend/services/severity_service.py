...
class SeverityEstimationService:
    ...
    def estimate_severity(image_bytes: bytes, is_healthy_label: bool = False) -> Dict[str, Any]:
        ...
        total_leaf_pixels = np.sum(total_plant_mask > 0)
        lesion_pixels = np.sum(lesion_mask > 0)

        if total_leaf_pixels < 1000:
            # Fallback if segment threshold is small
            affected_percentage = 18.5
        else:
            affected_percentage = float((lesion_pixels / total_leaf_pixels) * 100)
            affected_percentage = min(98.0, max(2.0, affected_percentage))
        ...
        if affected_percentage <= 20.0:
            severity_category = "Mild (0-20%)"
            severity_tier = 1
            action_urgency = "Monitor & Apply Preventive Bio-agents"
            color_code = "#22C55E"
